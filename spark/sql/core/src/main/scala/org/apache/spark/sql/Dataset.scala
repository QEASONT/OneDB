...
class Dataset[T] private[sql](
  ...
  def createODBIndex(keyDouble: Column, keyStr: Column, keyMetricM: Column, columnMetricMaxDis: Column, indexName: String): Unit = {
    //    val odbSimilarityExpression = ODBSimilarityExpression(function, key.expr, key.expr)
    CreateODBIndexCommand.createODBIndex(sparkSession, logicalPlan, keyDouble.toString,
      keyStr.toString, keyMetricM.toString, columnMetricMaxDis.toString,
      indexName, None)
  }
  ...
  def dropODBIndex(key: Column): Unit = {
    DropODBIndexCommand.dropODBIndex(sparkSession, logicalPlan, key.toString)
  }
  ...
  /*
   * ODB similarity with range query
   */

  def odbSimilarityWithRangeSearch(center: MetricData, radius: Double, queryM: Array[Int],
                                   key: Column): DataFrame = withPlan {
    val condition = ODBSimilarityRangeExpression(key.expr, center, radius, queryM)
    Filter(condition, logicalPlan)
  }
