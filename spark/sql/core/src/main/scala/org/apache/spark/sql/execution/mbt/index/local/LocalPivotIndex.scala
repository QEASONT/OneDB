...
import org.apache.spark.sql.execution.mbt.partition.local.LocalPivotPartitioner
...
object LocalPivotIndex {
  def buildIndex(dataKeyValuePair: Array[(Double, Point[Any])], function: MBTSimilarityFunction): LocalPivotIndex = {
    val (_, partitioner) = LocalPivotPartitioner.partition(dataKeyValuePair, function)
