...
case class RTreeLeafEntry(shape: Shape, m_data: Int, size: Int) extends RTreeEntry {
  ...
  override def dist(x: Any, queryM: Array[(Int, Int)]): Double = -1
  ...
  def circleRange(origin: Shape, r: Double): List[(Shape, Int)] = {
    ...
    while (st.nonEmpty) {
      ...
      if (!now.isLeaf) {
        now.m_child.foreach {
          case RTreeInternalEntry(mbr, node) =>
            if (origin.dist(mbr) <= r) st.push(node)
        }
      } else {
        now.m_child.foreach {
          case RTreeLeafEntry(shape, m_data, _) =>
            if (origin.dist(shape) <= r) ans += ((shape, m_data))
        }
      }
    }
    ans.toList
...
object RTree {
  def apply(entries: Array[(Point[Any], Int)], max_entries_per_node: Int): RTree = {
