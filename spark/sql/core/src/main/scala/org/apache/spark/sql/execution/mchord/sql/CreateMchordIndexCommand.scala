...
case class CreateMchordIndexCommand(tableName: TableIdentifier, column: String, indexName: String)
  extends RunnableCommand {

  override def run(sparkSession: SparkSession): Seq[Row] = {
    val table = sparkSession.table(tableName)
    CreateMchordIndexCommand.createMchordIndex(sparkSession, table.logicalPlan, column, indexName,
      Some(tableName))
    Seq.empty[Row]
  }
}

object CreateMchordIndexCommand extends Logging {
  def createMchordIndex(sparkSession: SparkSession, logicalPlan: LogicalPlan, column: String,
    ...
    if (catalog.lookupMchordIndex(tableName, indexName, project).isEmpty) {
      val child = sparkSession.sessionState.executePlan(logicalPlan).sparkPlan
      val indexedRelation = MchordIndexedRelation(child, attribute)()
