...
sealed abstract class Node[A](var childrenNum: Int = -1) extends Serializable {
  def center: MetricData
}

case class Branch[A](var children: Vector[RoutingObject[A]],
                     var center: MetricData) extends Node[A] {
...
case class Leaf[A](var children: Vector[Entry[A]],
                   var center: MetricData, var num: Int = -1) extends Node[A] {
  ...
  def split(m: Int): Vector[RoutingObject[A]] = {
    ...
    val op1Leaf = new Leaf[A](N1.toVector, op1.center, -1);
    val op1RoutingObject = new RoutingObject[A](op1.center, op1Leaf, 0, op1Radius);
    val op2Leaf = new Leaf[A](N2.toVector, op2.center, -1);
...
case class RoutingObject[A](var center: MetricData, var CoveringTree: Node[A],
                            var DistanceFromParent: Double, var radius: Double)

case class Entry[A](var center: MetricData, var DistanceFromParent: Double, var value: A)