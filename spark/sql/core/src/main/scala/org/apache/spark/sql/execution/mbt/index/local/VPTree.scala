 ...
 */
final class VPTree(val root: Tree, distFunction: MBTSimilarityFunction) extends Serializable {
...
object VPTree {
  ...
  def apply(items: IndexedSeq[Point[Any]], leafSize: Int, distFunction: MBTSimilarityFunction): VPTree =
    new VPTree(mkNode(items, leafSize, distFunction), distFunction)
  ...
  final case class Node(point: Point[Any], radius: Double,
                        size: Int, in: Tree, out: Tree, distFunction: MBTSimilarityFunction) extends Tree {
    def toSeq: IndexedSeq[Point[Any]] = in.toSeq ++ out.toSeq

    def approxNear(t: Point[Any]): Point[Any] = {
      val d = point.minDist(t, distFunction)
      if (d < radius) {
        in.approxNear(t)
      } else {
        out.approxNear(t)
  ...
  final case class Leaf(points: IndexedSeq[Point[Any]], distFunction: MBTSimilarityFunction) extends Tree {
    def size: Int = points.length

    def toSeq: IndexedSeq[Point[Any]] = points

    def approxNear(t: Point[Any]): Point[Any] = points minBy (p => p.minDist(t, distFunction))
    ...
    def approxNearN(t: Point[Any], n: Int): IndexedSeq[Point[Any]] =
      if (n <= 0) {
        ...
        points sortBy (p => p.minDist(t, distFunction)) take n
      }

    def nearN(t: Point[Any], maxDist: Double): IndexedSeq[Point[Any]] =
      points filter { p => t.minDist(p, distFunction) <= maxDist }
  }

  def mkNode(items: IndexedSeq[Point[Any]], leafSize: Int, distFunction: MBTSimilarityFunction): Tree = {
    if (items.length <= leafSize) {
      Leaf(items, distFunction)
    } else {
      val vp = items(util.Random.nextInt(items.length))

      val radius = {
        val numSamples = math.sqrt(items.length).toInt * 2
        val distances = pickSample(items, numSamples).map(i => vp.minDist(i, distFunction)).toArray
        java.util.Arrays.sort(distances)
        distances(distances.length / 2)
      }

      val (in, out) = items partition { item => item.minDist(vp, distFunction) < radius }

      if (in.length == 0) {
        Leaf(out, distFunction)
      } else if (out.length == 0) {
        Leaf(in, distFunction)
      } else {
        Node(vp, radius, items.length, mkNode(in, leafSize, distFunction), mkNode(out, leafSize, distFunction), distFunction)
      }
    }
  }
  ...
  def pickSample(items: IndexedSeq[Point[Any]], size: Int): IndexedSeq[Point[Any]] =
    if (items.length <= size) {
      ...
      IndexedSeq.fill(size)(items(util.Random.nextInt(items.length)))
    }

  def balance(t: Tree): List[(Int, Int)] = t match {
    case Leaf(_, distFunction) => Nil
    case Node(_, _, _, in, out, distFunction) => List((in.size, out.size)) ::: balance(in) ::: balance(out)
  }
