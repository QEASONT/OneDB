...
import org.apache.spark.sql.execution.odb.index.local.VPTree

case class LocalPivotPartitioner(points: Array[(Double, Point[Any])])
  extends Partitioner {

  override def numPartitions: Int = 0

  val vpTree = VPTree(points.map(_._2), ODBConfigConstants.PIVOT_INDEX)
...
object LocalPivotPartitioner {
  def partition(points: Array[(Double, Point[Any])]):
