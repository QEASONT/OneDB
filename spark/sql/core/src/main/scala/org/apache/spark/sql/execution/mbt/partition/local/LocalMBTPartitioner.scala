...
case class LocalMBTPartitioner(dataKeyValuePair: Array[(Double, Point[Any])]) extends Partitioner {
  ...
  private def buildBPlusTree(): MemoryBPlusTree[Double, Any] = {
    val bPlusTree = new MemoryBPlusTree[Double, Any](MBTConfigConstants.BPlusTreeOrder)
...
object LocalMBTPartitioner {
  def partition(dataKeyValuePair: Array[(Double, Point[Any])]):
  (Array[Array[Point[Any]]], LocalMBTPartitioner) = {
    val partitioner = new LocalMBTPartitioner(dataKeyValuePair)
