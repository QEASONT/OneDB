...
import org.apache.spark.sql.execution.mbt.partition.local.LocalSimplePartitioner
...
object LocalSimpleIndex {
  def buildIndex(dataKeyValuePair: Array[(Double, Point[Any])], function: MBTSimilarityFunction): LocalSimpleIndex = {
    val (_, partitioner) = LocalSimplePartitioner.partition(dataKeyValuePair, function)
