...
import org.apache.spark.sql.execution.odb.index.global.MemoryBPlusTree
...
case class GlobalBPlusTreePartitioner(@transient dataKeyValuePair
  ...
  private def buildBPlusTree(): MemoryBPlusTree[Double, Any] = {
    ...
    val bPlusTree = new MemoryBPlusTree[Double, Any](ODBConfigConstants.GlobalBPlusTreeOrder)
  ...
  override def getPartition(key: Any): Int = {
    ...
    val exactPartition = BPlusTree.samplelookup(key.asInstanceOf[Double])
...
object GlobalBPlusTreePartitioner {
  private val sampleSize = ODBConfigConstants.SAMPLE_SIZE

  def partition(dataKeyValuePair: RDD[(Double, Point[Any])]):
  (RDD[(Double, Point[Any])], GlobalBPlusTreePartitioner) = {
    //    val pairedDataRDD = dataKeyValuePair.map(x => (x, None))
    val partitioner = new GlobalBPlusTreePartitioner(dataKeyValuePair,
      sampleSize)
    val shuffled = new ShuffledRDD[Double, Point[Any], Point[Any]](dataKeyValuePair, partitioner)
    val aaa = shuffled.mapPartitionsWithIndex {
      case (index, iter) =>
