...
import org.apache.spark.sql.execution.mbt.partition.local.LocalMTreePartitioner
...
object LocalMTreeIndex {
  def buildIndex(data: Array[(Double, Point[Any])], distanceFunction: MBTSimilarityFunction): LocalMTreeIndex = {
    val (_, partitioner) = LocalMTreePartitioner.partition(data, distanceFunction)
