...
class SparkSqlAstBuilder(conf: SQLConf) extends AstBuilder(conf) {
   ...
   */
  override def visitCreateODBIndex(ctx: CreateODBIndexContext): LogicalPlan = withOrigin(ctx) {
    CreateODBIndexCommand(visitTableIdentifier(ctx.tableIdentifier), ctx.columnDouble.getText, ctx.columnString.getText,
      ctx.columnMetricM.getText,
      ctx.columnMetricMaxDis.getText,
      ctx.indexIdentifier.getText
    )
  }

  /*
   * Drop ODB index, returning a [[DropODBIndexCommand]] logical plan
   */
  override def visitDropODBIndex(ctx: DropODBIndexContext): LogicalPlan = withOrigin(ctx) {
    DropODBIndexCommand(visitTableIdentifier(ctx.tableIdentifier), ctx.indexIdentifier.getText)
  }

  /*
   * Show all ODB indexes, returning a [[ShowODBIndexes]] logical plan
   */
  override def visitShowODBIndexes(ctx: ShowODBIndexesContext): LogicalPlan = withOrigin(ctx) {
    ShowODBIndexesCommand()
  }
