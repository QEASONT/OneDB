...
case class Branch[A](var children: Vector[RoutingObject[A]],
                     var center: Point[Any], distFunction: MBTSimilarityFunction) extends Node[A] {
  def split(m: Int): Vector[RoutingObject[A]] = {
    ...
    for (i <- 0 to ChildrenArray.length - 1) {
      for (j <- i + 1 to ChildrenArray.length - 1) {
        val distance = ChildrenArray(j).center.minDist(ChildrenArray(i).center, distFunction);
    ...
    breakable {
      while (!ChildrenArray.isEmpty) {
        ...
        if (N2.length >= 2 && N1.length + ChildrenArray.length <= 2) {
          for (i <- 0 to ChildrenArray.length - 1) {
            var op1dist = ChildrenArray(i).center.minDist(op1.center, distFunction);
          ...
          for (i <- 0 to ChildrenArray.length - 1) {
            var op2dist = ChildrenArray(i).center.minDist(op2.center, distFunction);
        ...
        var op1dist = ChildrenArray(0).center.minDist(op1.center, distFunction);
        var op2dist = ChildrenArray(0).center.minDist(op2.center, distFunction);
        if (op1dist <= op2dist) {
    ...
    var op1Radius = -1.0;
    for (c <- N1) {
      val CurrentDistance = c.center.minDist(op1.center, distFunction);
    ...
    var op2Radius = -1.0;
    for (c <- N2) {
      val CurrentDistance = c.center.minDist(op2.center, distFunction);
      c.DistanceFromParent = CurrentDistance;
      if (CurrentDistance + c.radius > op2Radius) op2Radius = CurrentDistance + c.radius;
    }
    val op1Branch = new Branch[A](N1.toVector, op1.center, distFunction);
    val op1RoutingObject = new RoutingObject[A](op1.center, op1Branch, 0, op1Radius);
    val op2Branch = new Branch[A](N2.toVector, op2.center, distFunction);
...
case class Leaf[A](var children: Vector[Entry[A]],
                   var center: Point[Any], var num: Int = -1, distFunction: MBTSimilarityFunction) extends Node[A] {
  ...
  def split(m: Int): Vector[RoutingObject[A]] = {
    ...
    val op1Leaf = new Leaf[A](N1.toVector, op1.center, -1, distFunction);
    val op1RoutingObject = new RoutingObject[A](op1.center, op1Leaf, 0, op1Radius);
    val op2Leaf = new Leaf[A](N2.toVector, op2.center, -1, distFunction);
