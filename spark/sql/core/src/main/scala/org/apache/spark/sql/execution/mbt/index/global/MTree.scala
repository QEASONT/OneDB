 ...
 */
package org.apache.spark.sql.execution.mbt.index.global
...
case class MTree[A](
                     var InnerOrder: Int, var LeafOrder: Int, distanceFunction: MBTSimilarityFunction) {
  var root: Node[A] = new Leaf[A](Vector.empty, null, -1, distanceFunction)

  def insert(entry: Entry[A]): Unit = {
    insertRecursion(entry) match {
      case Left(ro) => root = new Branch[A](ro, null, distanceFunction);
      case Right(_) =>
    };
    ...
    Either[Vector[RoutingObject[A]], Double] = {
      curr match {
        case current: Leaf[A] =>
          ...
          if (current != root) entry.DistanceFromParent = current.center.minDist(entry.center, distanceFunction);
        ...
        case current: Branch[A] =>
          ...
          for (i <- 0 to current.children.length - 1) {
            val CurrentDistance = current.children(i).center.minDist(entry.center, distanceFunction);
          ...
          insertRecursion(entry, current.children(MinIndex).CoveringTree) match {
            case Left(ro) =>
              if (current != root) {
                ro.foreach(e => e.DistanceFromParent = e.center.minDist(current.center, distanceFunction));
              }
              ...
              if (current != root) {
                for (c <- current.children) {
                  if (current.center.minDist(c.center, distanceFunction) + c.radius > newRadius) {
                    newRadius = current.center.minDist(c.center, distanceFunction) + c.radius
                  };
                }
              }
    ...
    Either[(ArrayBuffer[Node[A]], Double, Boolean), Double] = {
      if (curr.isInstanceOf[Leaf[A]]) {
        ...
        for (c <- current.children) {
          ...
          if (c.center.minDist(entry.center, distanceFunction) <= c.radius) {
  ...
  def update(OldEntry: Entry[A], NewEntry: Entry[A]): Boolean = {
    def updateRecursion(OldEntry: Entry[A], NewEntryValue: A, curr: Node[A] = root): Boolean = {
      if (curr.isInstanceOf[Leaf[A]]) {
        ...
        var result = -1;
        for (c <- current.children) {
          if (c.center.minDist(OldEntry.center, distanceFunction) <= c.radius) {
      ...
      if (curr == root) {
        if (curr.isInstanceOf[Leaf[A]]) {
          ...
          for (e <- current.children) {
            if (SearchPoint.minDist(e.center, distanceFunction) <= SearchRadius) {
              found = found :+ e;
            }
          }
          return found;
          ...
          for (or <- current.children) {
            val distanceQP = SearchPoint.minDist(or.center, distanceFunction);
      ...
      while (!PR.isEmpty) {
        ...
        if (NextNode.isInstanceOf[Leaf[A]]) {
          ...
          for (oj <- current.children) {
            val dist = oj.center.minDist(Q, distanceFunction);
            if (dist <= dk) {
          ...
          for (or <- current.children) {
            val dist = or.center.minDist(Q, distanceFunction);
  ...
  def estimatedKNNDist(Q: Point[Any], k: Int): Double = {
    ...
    breakable {
      while (!PR.isEmpty) {
        ...
        if (NextNode.isInstanceOf[Leaf[A]]) {
          val current = NextNode.asInstanceOf[Leaf[A]];
          val dist = current.children.map(x => x.center.minDist(Q, distanceFunction)).sorted
          dk = dist.last
          break
        } else {
          if (k < NextNode.childrenNum && NextNode.childrenNum < 1.5 * k) {
            val current = NextNode.asInstanceOf[Branch[A]];
            val dist = current.children.map(x => x.center.minDist(Q, distanceFunction) + x.radius).sorted
            dk = dist.last
            break
          } else {
            val current = NextNode.asInstanceOf[Branch[A]];
  ...
  def traverse(radius: Double = -1, curr: Node[A] = root): Double = {
    if (curr.isInstanceOf[Leaf[A]]) {
      ...
      if (current != root) {
        for (k <- current.children) {
          val CurrentDistance = k.center.minDist(current.center, distanceFunction);
      ...
      if (current != root) {
        var newRadius = Double.NegativeInfinity;
        for (k <- current.children) {
          val CurrentDistance = k.center.minDist(current.center, distanceFunction) + k.radius;
          if (CurrentDistance > newRadius) newRadius = CurrentDistance;
        }
        return newRadius
      }
  ...
  def counter(radius: Double, curr: Node[A] = root): Int = {
    if (curr.isInstanceOf[Leaf[A]]) {
      ...
      for (i <- 0 to current.children.length - 1) {
        if (current != root) {
          if (radius < current.children(i).center.minDist(current.center, distanceFunction)) {
            c = c + 1;
          }
        }
      }
      ...
      var flag = false;
      for (i <- 0 to current.children.length - 1) {
        if (current != root && current.center.minDist(current.children(i).center, distanceFunction)
  ...
  def find(pt: Entry[A], curr: Node[A] = root): Boolean = {
    if (curr.isInstanceOf[Leaf[A]]) {
      ...
      for (k <- curr.asInstanceOf[Branch[A]].children) {
        if (k.center.minDist(pt.center, distanceFunction) <= k.radius) {
          // println("Child: " + i);
  ...
  def getSampleLeafIndex(pt: Entry[A], curr: Node[A] = root): Int = {
    if (curr.isInstanceOf[Leaf[A]]) {
      ...
      var i = 0
      for (k <- curr.asInstanceOf[Branch[A]].children) {
        val dist = k.center.minDist(pt.center, distanceFunction)
        if (dist <= minDist) {
          minDist = dist
          minIndex = i
        }
      ...
      val res = getSampleLeafIndex(pt,
        curr.asInstanceOf[Branch[A]].children(minIndex).CoveringTree)
      res;
    }
  }
  ...
  //  def getSampleDistanceFromParent(pt: Entry[A], curr: Node[A] = root): Double = {
  //    if (curr.isInstanceOf[Leaf[A]]) {
  //      curr.asInstanceOf[Leaf[A]].center.minDist(pt.center)
  //    } else {
  //      var i = 0
  //      var minDist = Double.PositiveInfinity
  //      var minIndex = -1
  //      for (k <- curr.asInstanceOf[Branch[A]].children) {
  //        val dist = k.center.minDist(pt.center)
  //        if (dist <= minDist) {
  //          minDist = dist
  //          minIndex = i
  //        }
  //        i = i + 1;
  //      }
  //      val res = getSampleDistanceFromParent(pt,
  //        curr.asInstanceOf[Branch[A]].children(minIndex).CoveringTree)
  //      res;
  //    }
  //  }
