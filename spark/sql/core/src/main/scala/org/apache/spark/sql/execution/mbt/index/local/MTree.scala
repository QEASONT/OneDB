 ...
 */
package org.apache.spark.sql.execution.mbt.index.local
...
case class MTree[A](var InnerOrder: Int, var LeafOrder: Int, distanceFunction: MBTSimilarityFunction) {
  ...
  def RangeSearch(SearchPoint: Point[Any], SearchRadius: Double): Vector[(Entry[A], Double)] = {
    def RangeSearchRecursion(SearchPoint: Point[Any], SearchRadius: Double,
                             curr: Node[A] = root,
                             DistanceQP: Double = 0): Vector[(Entry[A], Double)] = {
      if (curr == root) {
        if (curr.isInstanceOf[Leaf[A]]) {
          val current = curr.asInstanceOf[Leaf[A]];
          var found = Vector[(Entry[A], Double)]();
          for (e <- current.children) {
            val dist = SearchPoint.minDist(e.center, distanceFunction)
