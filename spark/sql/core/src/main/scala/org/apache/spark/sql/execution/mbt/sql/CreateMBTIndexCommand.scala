...
case class CreateMBTIndexCommand(distanceFunction: MBTSimilarityFunction,
                                 tableName: TableIdentifier, column: String, indexName: String)
  extends RunnableCommand {

  override def run(sparkSession: SparkSession): Seq[Row] = {
    val table = sparkSession.table(tableName)
    CreateMBTIndexCommand.createMBTIndex(distanceFunction, sparkSession, table.logicalPlan, column, indexName,
      Some(tableName))
    Seq.empty[Row]
  }
}

object CreateMBTIndexCommand extends Logging {
  def createMBTIndex(distanceFunction: MBTSimilarityFunction, sparkSession: SparkSession,
                     logicalPlan: LogicalPlan, column: String,
    ...
    if (catalog.lookupMBTIndex(tableName, indexName, project).isEmpty) {
      val child = sparkSession.sessionState.executePlan(logicalPlan).sparkPlan
      val indexedRelation = MBTIndexedRelation(distanceFunction, child, attribute)()
