 ...
 */
package org.apache.spark.sql.execution.mbt.partition.local
...
import org.apache.spark.sql.execution.mbt.index.local.{Entry, MTree}

import scala.collection.mutable.ArrayBuffer

case class LocalMTreePartitioner(data: Array[(Double, Point[Any])], distanceFunction: MBTSimilarityFunction) extends Partitioner {
  ...
  private def buildMTree(): MTree[Int] = {
    val MTree = new MTree[Int](InnerOrder = MBTConfigConstants.LOCAL_M_TREE_INNER_ORDER,
      LeafOrder = MBTConfigConstants.LOCAL_M_TREE_LEAF_ORDER, distanceFunction = distanceFunction);
    data.foreach(point => {
      val e = new Entry[Int](point._2, 0, 0);
      MTree.insert(e)
    })
  ...
  def deletePoint(key: Point[Any]): List[(Point[Any], Double)] = {
    val e = new Entry[Int](key, 0, 0);
    val partitionIndex = getPartition(key)
  ...
  def insertPoint(key: Point[Any]): List[(Point[Any], Double)] = {
    val e = new Entry[Int](key, 0, 0);
    MTree.insert(e)
    //    point
    //    MTree.setLeafIndex(MTree.root, 0)
...
object LocalMTreePartitioner {
  def partition(data: Array[(Double, Point[Any])], distanceFunction: MBTSimilarityFunction):
  (Array[Array[Point[Any]]], LocalMTreePartitioner) = {
    val partitioner = new LocalMTreePartitioner(data, distanceFunction)
