...
import org.apache.spark.sql.execution.mbt.index.global.MemoryBPlusTree
...
case class GlobalBPlusTreePartitioner(@transient dataKeyValuePair
  ...
  private def buildBPlusTree(): MemoryBPlusTree[Double, Any] = {
    ...
    val bPlusTree = new MemoryBPlusTree[Double, Any](MBTConfigConstants.GlobalBPlusTreeOrder)
  ...
  override def getPartition(key: Any): Int = {
    ...
    val exactPartition = key match {
      case x: (Double, Point[Any]) => BPlusTree.samplelookup(x._1)
      case x: Double => BPlusTree.samplelookup(x)
    }
