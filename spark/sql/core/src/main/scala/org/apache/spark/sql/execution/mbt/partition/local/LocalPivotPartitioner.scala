...
import org.apache.spark.sql.execution.mbt.index.local.VPTree

case class LocalPivotPartitioner(points: Array[(Double, Point[Any])], function: MBTSimilarityFunction)
  extends Partitioner {

  override def numPartitions: Int = 0

  val vpTree = VPTree(points.map(_._2), MBTConfigConstants.PIVOT_INDEX, function)
  ...
  def getResultWithThreshold(key: Point[Any], threshold: Double): List[(Point[Any], Double)] = {
    vpTree.nearest(key, threshold).map(p => (p, p.minDist(key, function))).toList
  }
...
object LocalPivotPartitioner {
  def partition(points: Array[(Double, Point[Any])], function: MBTSimilarityFunction):
  (Array[Array[Point[Any]]], LocalPivotPartitioner) = {
    val partitioner = new LocalPivotPartitioner(points, function)
