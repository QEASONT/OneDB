...
case class LocalTrieIndex(partitioner: LocalTriePartitioner) extends LocalIndex {
  def getCandidates(key: Trajectory, distanceFunction: TrajectorySimilarity,
                    threshold: Double): List[Trajectory] = {
    partitioner.getCandidates(key, distanceFunction, threshold,
      0.0).filter(_._2 <= threshold).map(_._1)
  }
