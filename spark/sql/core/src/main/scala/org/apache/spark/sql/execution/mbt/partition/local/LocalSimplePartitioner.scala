...
import org.apache.spark.sql.execution.mbt.index.local.MemoryBPlusTree

import scala.math.{max, min}
import scala.collection.mutable.ArrayBuffer

case class LocalSimplePartitioner(dataKeyValuePair: Array[(Double, Point[Any])], function: MBTSimilarityFunction)
  extends Partitioner {
  ...
  def getResultWithThreshold(key: Point[Any], threshold: Double): List[(Point[Any], Double)] = {
    dataShuffled.flatMap(data => {
      data.map(p => (p, p.minDist(key, function)))
...
object LocalSimplePartitioner {
  def partition(dataKeyValuePair: Array[(Double, Point[Any])], function: MBTSimilarityFunction):
  (Array[Array[Point[Any]]], LocalSimplePartitioner) = {
    val partitioner = new LocalSimplePartitioner(dataKeyValuePair, function)
    ...
    partitioner.dataShuffled = shuffled.map(_._2.map(_._2)).toArray
    (null, partitioner)
  }
