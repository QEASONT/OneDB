 ...
 */
//sealed abstract class N ()extends Serializable
...
abstract class BPlusTree[K <% Ordered[K], +V] extends Serializable {
  ...
  protected def getPartitionValue(node: N, index: Int): V
  ...
  def samplelookup(key: K): (Boolean, Some[V], Int) = {
    def _lookup(n: N): (Boolean, Some[V], Int) =
      if (isLeaf(n)) {
        binarySearch(n, key) match {
          case index if index >= 0 => (true, Some(getPartitionValue(n, index)), index)
          case index => (false, Some(getPartitionValue(n, index)), -(index + 1))
        }
      } else {
        binarySearch(n, key) match {
          case index if index >= 0 => _lookup(getBranch(n, index + 1))
  ...
  def setLeafIndex() {
    ...
    while (n != nul) {
      getKeys(n).foreach(m => lookup(m) match {
        case (true, leaf, index) => setValue(leaf, index, i)
      })
