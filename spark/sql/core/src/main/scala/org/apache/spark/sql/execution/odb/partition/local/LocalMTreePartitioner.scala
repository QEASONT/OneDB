...
import org.apache.spark.sql.execution.odb.index.local.{Entry, MTree}

import scala.collection.mutable.ArrayBuffer

case class LocalMTreePartitioner(data: Array[(Double, Point[Any])]) extends Partitioner {
  ...
  private def buildMTree(): MTree[Int] = {
    val MTree = new MTree[Int](InnerOrder = ODBConfigConstants.LOCAL_M_TREE_INNER_ORDER,
      LeafOrder = ODBConfigConstants.LOCAL_M_TREE_LEAF_ORDER);
    data.foreach(point => {
...
object LocalMTreePartitioner {
  def partition(data: Array[(Double, Point[Any])]):
