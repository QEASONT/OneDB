...
abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  object MetricScans extends Strategy {

    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {


      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[MetricSimilarityWithKNNExpression]) =>
        val expressionWithThreshold = filters.find(e =>
            e.isInstanceOf[MetricSimilarityWithKNNExpression]).get
          .asInstanceOf[MetricSimilarityWithKNNExpression]
        val count = expressionWithThreshold.count
        val expression = expressionWithThreshold.similarity
        val function = expression.function
        val (leftQuery, rightKey) = expression.met1 match {
          case Literal(v, _) => (v.asInstanceOf[Point[Any]], expression.met2)
          case _ => (expression.met2.asInstanceOf[Literal].value.asInstanceOf[Point[Any]],
            expression.met1)
        }
        val exec = MetricSimilarityWithKNNSearchExec(leftQuery,
          rightKey, function, count, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[MetricSimilarityWithKNNExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[MetricSimilarityRangeExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[MetricSimilarityRangeExpression]).get
          .asInstanceOf[MetricSimilarityRangeExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center
        val radius = circleExpression.radius

        val exec = MetricRangeSearchExec(center,
          expression, radius, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[MetricSimilarityRangeExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[MetricDeleteExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[MetricDeleteExpression]).get
          .asInstanceOf[MetricDeleteExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center

        val exec = MetricDeleteExec(center,
          expression, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[MetricDeleteExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[MetricInsertExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[MetricInsertExpression]).get
          .asInstanceOf[MetricInsertExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center

        val exec = MetricInsertExec(center,
          expression, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[MetricInsertExpression]),
          _ => exec) :: Nil
  ...
  object AMDSScans extends Strategy {

    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {


      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[AMDSSimilarityWithKNNExpression]) =>
        val expressionWithThreshold = filters.find(e =>
            e.isInstanceOf[AMDSSimilarityWithKNNExpression]).get
          .asInstanceOf[AMDSSimilarityWithKNNExpression]
        val count = expressionWithThreshold.count
        val expression = expressionWithThreshold.similarity
        val function = expression.function
        val (leftQuery, rightKey) = expression.met1 match {
          case Literal(v, _) => (v.asInstanceOf[AMDSPoint[Any]], expression.met2)
          case _ => (expression.met2.asInstanceOf[Literal].value.asInstanceOf[AMDSPoint[Any]],
            expression.met1)
        }
        val exec = AMDSSimilarityWithKNNSearchExec(leftQuery,
          rightKey, function, count, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[AMDSSimilarityWithKNNExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[AMDSSimilarityRangeExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[AMDSSimilarityRangeExpression]).get
          .asInstanceOf[AMDSSimilarityRangeExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center
        val radius = circleExpression.radius

        val exec = AMDSRangeSearchExec(center,
          expression, radius, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[AMDSSimilarityRangeExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[AMDSDeleteExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[AMDSDeleteExpression]).get
          .asInstanceOf[AMDSDeleteExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center

        val exec = AMDSDeleteExec(center,
          expression, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[AMDSDeleteExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[AMDSInsertExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[AMDSInsertExpression]).get
          .asInstanceOf[AMDSInsertExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center

        val exec = AMDSInsertExec(center,
          expression, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[AMDSInsertExpression]),
          _ => exec) :: Nil
  ...
  object MBTScans extends Strategy {

    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {


      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[MBTSimilarityWithKNNExpression]) =>
        val expressionWithThreshold = filters.find(e =>
            e.isInstanceOf[MBTSimilarityWithKNNExpression]).get
          .asInstanceOf[MBTSimilarityWithKNNExpression]
        val count = expressionWithThreshold.count
        val expression = expressionWithThreshold.similarity
        val function = expression.function
        val (leftQuery, rightKey) = expression.met1 match {
          case Literal(v, _) => (v.asInstanceOf[MBTPoint[Any]], expression.met2)
          case _ => (expression.met2.asInstanceOf[Literal].value.asInstanceOf[MBTPoint[Any]],
            expression.met1)
        }
        val exec = MBTSimilarityWithKNNSearchExec(leftQuery,
          rightKey, function, count, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[MBTSimilarityWithKNNExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[MBTSimilarityRangeExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[MBTSimilarityRangeExpression]).get
          .asInstanceOf[MBTSimilarityRangeExpression]
        val function = circleExpression.function
        val expression = circleExpression.similarity
        val center = circleExpression.center
        val radius = circleExpression.radius

        val exec = MBTRangeSearchExec(function, center,
          expression, radius, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[MBTSimilarityRangeExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[MBTDeleteExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[MBTDeleteExpression]).get
          .asInstanceOf[MBTDeleteExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center
        val function = circleExpression.function
        val exec = MBTDeleteExec(function, center,
          expression, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[MBTDeleteExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[MBTInsertExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[MBTInsertExpression]).get
          .asInstanceOf[MBTInsertExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center
        val function = circleExpression.function
        val exec = MBTInsertExec(function, center,
          expression, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[MBTInsertExpression]),
          _ => exec) :: Nil
  ...
  object ODBScans extends Strategy {

    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {


      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[ODBSimilarityWithKNNExpression]) =>
        val expressionWithThreshold = filters.find(e =>
            e.isInstanceOf[ODBSimilarityWithKNNExpression]).get
          .asInstanceOf[ODBSimilarityWithKNNExpression]
        val expression = expressionWithThreshold.similarity
        val count = expressionWithThreshold.count
        val center = expressionWithThreshold.centers
        val queryM = expressionWithThreshold.queryM


        val exec = ODBSimilarityWithKNNSearchExec(center,
          expression, count, queryM, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[ODBSimilarityWithKNNExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[ODBSimilarityRangeExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[ODBSimilarityRangeExpression]).get
          .asInstanceOf[ODBSimilarityRangeExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.centers
        val radius = circleExpression.radius
        val queryM = circleExpression.queryM

        val exec = ODBRangeSearchExec(center,
          expression, radius, queryM, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[ODBSimilarityRangeExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[ODBDeleteExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[ODBDeleteExpression]).get
          .asInstanceOf[ODBDeleteExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center
        val exec = ODBDeleteExec(center,
          expression, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[ODBDeleteExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[ODBInsertExpression]) =>
        val circleExpression = filters.find(e =>
            e.isInstanceOf[ODBInsertExpression]).get
          .asInstanceOf[ODBInsertExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center
        val exec = ODBInsertExec(center,
          expression, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[ODBInsertExpression]),
          _ => exec) :: Nil
