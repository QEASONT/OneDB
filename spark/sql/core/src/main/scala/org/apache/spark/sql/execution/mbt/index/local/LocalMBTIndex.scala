...
import org.apache.spark.sql.catalyst.expressions.mbt.index.LocalIndex
import org.apache.spark.sql.execution.mbt.partition.local.LocalMBTPartitioner

case class LocalMBTIndex(partitioner: LocalMBTPartitioner) extends LocalIndex {
...
object LocalMBTIndex {
  def buildIndex(dataKeyValuePair: Array[(Double, Point[Any])]): LocalMBTIndex = {
    val (_, partitioner) = LocalMBTPartitioner.partition(dataKeyValuePair)
    //    println(partitioner.numPartitions)
    //    val aa = partitioner.numPartitions
    LocalMBTIndex(partitioner)
