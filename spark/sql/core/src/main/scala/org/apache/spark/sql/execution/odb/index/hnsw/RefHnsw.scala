...
import org.apache.spark.sql.execution.odb.index.hnsw.RefHnsw.{CandidateList, EntryPoint, NeighborList}
 ...
 */
class RefHnsw(override val distanceOrd: Ordering[Double],
              val m: Int = 16,
  ...
  private var points: Array[Point[Any]] = _
  ...
  private def addVector(point: Point[Any]): Int = {
    val index = nextIndex
    points(index) = point
    nextIndex += 1
    index
  ...
  private def searchLayer(q: Point[Any], ep: Int, ef: Int, lc: Int): Seq[Candidate] = {
    ...
    def addCandidate(index: Int): Unit = {
      val candidate = Candidate(index, points(index).minDist(q))
      W.push(candidate)
      C.push(candidate)
    }
        ...
        if (distanceOrd.gt(c.distance, f.distance)) {
          ...
          neighbourhood.arr.foreach { e =>
            ...
            if (!v.contains(e)) {
              ...
              if (distanceOrd.lt(points(e).minDist(q), points(f.index).minDist(q)) || W.size < ef) {
                // C ← C ⋃ e
                // W ← W ⋃ e
                addCandidate(e)
                // if │W│ > ef
  ...
  def adjacencyListToCSR(): (Array[Int], Array[Int]) = {

    val n = neighborGraph.length
    val xadj = ArrayBuffer[Int](0)
    val adjncy = ArrayBuffer[Int]()

    for (neighbors <- neighborGraph) {
      adjncy ++= neighbors(0).arr
      xadj += adjncy.length
    }
  ...
  private def selectNeighbors1Alg3(candidates: CandidateList, M: Int): Seq[Candidate] = {
    candidates.toList.sorted(candidateOrd).take(M)
  }

  // SELECT-NEIGHBORS(e, eConn, Mmax, lc)
  private def selectNeighbors2Alg3(e: Int, C: Seq[Int], M: Int): Seq[Candidate] = {
    val point = points(e)
    val candidateList = CandidateList(candidateOrd.reverse)
    C.foreach { candidate =>
      candidateList.push(Candidate(candidate, points(candidate).minDist(point)))
    }
  ...
  override def add(point: Point[Any]): Unit = {
    //    Utils.assert(point.dimension == dimension, s"input vector dim=${point.dimension}, knn dim=$dimension")
    val q = point
    val index = addVector(point)
  ...
  override def knnWithDis(vector: Point[Any], k: Int): Seq[(Point[Any], Double)] = {
    ...
    // W ← SEARCH-LAYER(q, ep, ef, lc =0)
    W.pushAll(searchLayer(q, ep, ef, 0))
    W.take(k).map(c => (points(c.index), c.distance))
  }
}
...
object RefHnsw {
  ...
  case class CandidateList(ord: Ordering[Candidate]) {
    private val lst = new mutable.HashSet[Int]()
    private val pq = new mutable.PriorityQueue[Candidate]()(ord)

    def push(candidates: Candidate*): Unit = pushAll(candidates)

    def pushAll(candidates: Seq[Candidate]): Unit = {
    ...
    def toList: Seq[Candidate] = pq.toList

    def size: Int = pq.size

    def take(n: Int): Seq[Candidate] = {
