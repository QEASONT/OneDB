...
case class MTree[A](
  ...
  var root: Node[A] = new Leaf[A](Vector.empty, null, -1)
  ...
  def RangeSearch(SearchPoint: MetricData, SearchRadius: Double): Vector[Entry[A]] = {
    def RangeSearchRecursion(SearchPoint: MetricData, SearchRadius: Double,
  ...
  def indexRangeSearch(SearchPoint: MetricData, SearchRadius: Double): Vector[Int] = {
    def RangeSearchRecursion(SearchPoint: MetricData, SearchRadius: Double,
  ...
  def kNNSearch(Q: MetricData, k: Int): Array[(Entry[A], Double)] = {
  ...
  def estimatedKNNDist(Q: MetricData, k: Int): Double = {
