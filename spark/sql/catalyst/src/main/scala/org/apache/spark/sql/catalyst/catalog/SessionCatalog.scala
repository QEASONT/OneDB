...
class SessionCatalog(
  ...
  def getCurrentDatabase: String = synchronized {
    currentDb
  }
  ...
  def createIndex(tableName: Option[TableIdentifier], indexName: String, plan: LogicalPlan,
                  indexedRelation: ODBIndexedRelation): Unit = {
    if (odbIndexRegistry.lookupIndex(indexName, plan).nonEmpty) {
      logWarning("Index exists!")
    } else {
      odbIndexRegistry.registerIndex(indexName, tableName.map(_.table), plan, indexedRelation)
    }
  }
   ...
   * create index on the table ODB
   */
  def createIndex(tableName: String, indexName: String, plan: LogicalPlan,
                  indexedRelation: ODBIndexedRelation): Unit = {
    if (odbIndexRegistry.lookupIndex(indexName, plan).nonEmpty) {
      logWarning("Index exists!")
    } else {
      odbIndexRegistry.registerIndex(indexName, Option(tableName), plan, indexedRelation)
    }
  }
   ...
   * lookup index odb
   */
  def lookupODBIndex(tableName: Option[TableIdentifier], indexName: String,
                     plan: LogicalPlan): Option[ODBIndexEntry] = {
   ...
   * lookup index odb
   */
  def lookupODBIndex(plan: LogicalPlan): Option[ODBIndexEntry] = {
   ...
   * drop index odb
   */
  def dropODBIndex(plan: LogicalPlan): Unit = {
    if (!odbIndexRegistry.dropIndex(plan)) {
   ...
   * drop index odb
   */
  def dropODBIndex(tableName: Option[TableIdentifier], indexName: String): Unit = {
    if (!odbIndexRegistry.dropIndex(indexName, tableName.map(_.table))) {
   ...
   * show indexes odb
   */
  def showODBIndexes(): Seq[String] = {
    odbIndexRegistry.showIndexes()
  }
