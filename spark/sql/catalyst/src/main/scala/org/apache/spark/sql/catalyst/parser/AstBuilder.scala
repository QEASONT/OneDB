...
class AstBuilder(conf: SQLConf) extends SqlBaseBaseVisitor[AnyRef] with Logging {
 ...
 null
  ...
  Literal = withOrigin(ctx) {
    // QT: needs to be test
    val metricM = ctx.metricm.coords.asScala.map(_.getText.toInt).toArray
    val queryM = ctx.querym.coords.asScala.map(_.getText.toInt).toArray
    val coords = ctx.obtcoords.coords.asScala.map(_.getText).toArray
    val res = metricM.zipWithIndex.map(x => {
      val i = x._2
      val m = metricM(i)
      m match {
        case 0 | 1 | 2 | 3 | 4 | 5 =>
          val doubleArray = coords(i).stripPrefix("'").stripSuffix("'").split(",").map(x => x.toDouble)
          ODBPoint[Any](doubleArray, i, m)
        case 6 =>
          val stringArray = coords(i).stripPrefix("'").stripSuffix("'")
          ODBPoint[Any](stringArray, i, m)
      }
    })
    Literal((res, queryM), DataTypes.NullType)
    //    if (ctx.obtcoords.coordsString != null) {
    //      Literal(ODBPoint[Any](ctx.mcoords.coordsString
    //        .toString().stripPrefix("'").stripSuffix("'")), DataTypes.NullType)
    //    } else {
    //      Literal(ODBPoint[Any](ctx.mcoords.coords
   ...
   */
  override def visitOdbSimilarityExpression
  (ctx: OdbSimilarityExpressionContext): ODBSimilarityExpression = withOrigin(ctx) {
    val left = if (ctx.leftTable != null) {
      expression(ctx.leftTable)
    } else {
      visitOdbPointExpression(ctx.leftMetricPoint)
    }
    val right = if (ctx.rightTable != null) {
      expression(ctx.rightTable)
    } else {
      visitOdbPointExpression(ctx.rightMetricPoint)
    }
    ODBSimilarityExpression(
      ODBSimilarityFunction.EUCLID, left, right)
  }
  ...
  ODBSimilarityWithKNNExpression = withOrigin(ctx) {
    val (centers, queryM) = visitOdbPointExpression(ctx.center).value.asInstanceOf[(Array[ODBPoint[Any]], Array[Int])]
    ODBSimilarityWithKNNExpression(
  ...
  //    ctx match {
  //      case function if function.EUCLID() != null => ODBSimilarityFunction.EUCLID
  //      case function if function.L1() != null => ODBSimilarityFunction.L1
  //      case function if function.COSINE() != null => ODBSimilarityFunction.COSINE
  //      case function if function.EDIT() != null => ODBSimilarityFunction.EDIT
  //    }
  //  }
