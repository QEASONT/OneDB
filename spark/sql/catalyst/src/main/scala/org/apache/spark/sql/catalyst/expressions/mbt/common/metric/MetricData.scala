...
case class MetricData(points: Array[Point[Any]]) {
  ...
  generatePivots(MBTConfigConstants.GLOBAL_INDEXED_PIVOT_COUNT).map(_._1)
  //  @transient
    ...
    for (id1 <- ids) {
      ...
      for (id2 <- ids) {
        if (id1 != id2) {
          ...
          maxDistance.add(candidate1.minDist(candidate2, null))
        }
      }
      i += 1
    }
  ...
  private def addOrNot(candidate: Point[Any], pivots: Array[(Point[Any], Int)], ignore: Boolean): Boolean = {
    ...
    for (o <- pivots) {
      if (!(ignore && o._1 == (candidate))) {
        val dist: Double = o._1.minDist(candidate, null)
