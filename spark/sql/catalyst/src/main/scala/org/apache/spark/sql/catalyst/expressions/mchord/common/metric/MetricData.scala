...
case class MetricData(points: Array[Point[Any]]) {
  ...
  generatePivots(MchordConfigConstants.GLOBAL_INDEXED_PIVOT_COUNT).map(_._1)
  //  @transient
  ...
  def generatePivots(pivotCount: Int): Array[(Point[Any], Int)] = {
    ...
    val ids = selectUnique(2000, random, null)
    var i = 0
  ...
  private def addOrNot(candidate: Point[Any], pivots: Array[(Point[Any], Int)],
    ...
    for (o <- pivots) {
      if (!(ignore && o._1 == (candidate))) {
        ...
        val current = maxDistance.mean + (maxDistance.standardDeviation * alpha)
        if (dist < current) {
          res = false
          return res
        }
