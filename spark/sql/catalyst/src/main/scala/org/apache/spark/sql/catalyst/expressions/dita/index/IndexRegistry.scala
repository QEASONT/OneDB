...
class IndexRegistry {
  ...
  def lookupIndex(plan: LogicalPlan): Option[IndexEntry] = synchronized {
    val aaa = indexEntries(1).plan.canonicalized
    val ddd = indexEntries(0).plan.canonicalized
    val bbb = plan.canonicalized
    val ccc = aaa.canonicalized == bbb.canonicalized
    val eee = ddd.canonicalized == bbb.canonicalized
