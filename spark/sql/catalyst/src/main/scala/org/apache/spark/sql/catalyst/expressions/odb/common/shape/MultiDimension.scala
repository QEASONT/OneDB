...
case class MultiDimension(low: Point[Any], high: Point[Any]) extends Shape {
  ...
  def expandToInclude(shape: Shape): MultiDimension = {
    shape match {
      case rect: MultiDimension =>
        ...
        MultiDimension(Point(newLow, low.metricIndex, low.metricValue), Point(newHigh, high.metricIndex, high.metricValue))
      case point: Point[_] =>
  ...
  def intersects(other: Shape): Boolean = {
    other match {
      ...
      case mbr: MultiDimension => intersects(mbr)
      case r: SimpleRange => r.intersects(this)
    }
  }

  def intersects(other: MultiDimension): Boolean = {
  ...
  def contains(rectangle: MultiDimension): Boolean = {
      ...
      queryM.foreach {
        case (mValue, mIndex) =>
          if (p.coord.asInstanceOf[Array[Double]](mIndex) < low.coord.asInstanceOf[Array[Double]](mIndex)) {
            ans += (low.coord.asInstanceOf[Array[Double]](mIndex) - p.coord.asInstanceOf[Array[Double]](mIndex)) * (low.coord.asInstanceOf[Array[Double]](mIndex) - p.coord.asInstanceOf[Array[Double]](mIndex))
          } else if (p.coord.asInstanceOf[Array[Double]](mIndex) > high.coord.asInstanceOf[Array[Double]](mIndex)) {
            ans += (p.coord.asInstanceOf[Array[Double]](mIndex) - high.coord.asInstanceOf[Array[Double]](mIndex)) * (p.coord.asInstanceOf[Array[Double]](mIndex) - high.coord.asInstanceOf[Array[Double]](mIndex))
          }
      }
    }
    else {
  ...
  def approxMinDist(other: MultiDimension): Double = {
    var ans = 0.0
    for (i <- low.coord.asInstanceOf[Array[Double]].indices) {
      var x = 0.0
      if (other.high.coord.asInstanceOf[Array[Double]](i) < low.coord.asInstanceOf[Array[Double]](i)) {
