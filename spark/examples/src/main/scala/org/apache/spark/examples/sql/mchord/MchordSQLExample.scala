...
object MchordSQLExample {
  ...
  def main(args: Array[String]) {
    ...
    MchordConfigConstants.DATA_TYPE = if (args.length > 1) args(1).toInt else 1
    MchordConfigConstants.KNN = if (args.length > 2) args(2).toInt else 8
    MchordConfigConstants.RANGE = if (args.length > 3) args(3).toDouble else 3800
    MchordConfigConstants.GLOBAL_INDEXED_PIVOT_COUNT = if (args.length > 4) args(4).toInt else 160
    ...
    if (MchordConfigConstants.DATA_TYPE == 0) {
      ...
      spark.sql("CREATE MCHORD INDEX metric1_index ON metric1 (metric)")
      var end = System.currentTimeMillis()
      println(s"Building Index time: ${end - start} ms")

      start = System.currentTimeMillis()
      val queryMetricStr = metricData.filter(t => t.id == 982).take(1)
        .map(point => s"METRICPOINT(${point.metric.mkString(",")})").head
      println(queryMetricStr)
      val radius = MchordConfigConstants.RANGE
//      print(s"SELECT * FROM metric1 WHERE metric1.metric IN METRICRANGE"
//        + s"(" + queryMetricStr + s",$radius)")
      spark.sql(s"SELECT * FROM metric1 WHERE metric1.metric IN METRICRANGE"
        + s"(" + queryMetricStr + s",$radius)").show()

      val k = MchordConfigConstants.KNN
      ...
      val queryMetricStr = metricData.filter(t => t.id == 982).take(1)
        .map(point => {
          val tmp: String = s"METRICPOINT('" + point.metric + "')"
          tmp
        }
        ).head
      println(queryMetricStr)
