...
object MBTDataFrameExample extends Logging {
  ...
  def main(args: Array[String]) {
    ...
    MBTConfigConstants.MBT_DISTANCE_TYPE = if (args.length > 2) args(2).toInt else 1
    MBTConfigConstants.MBT_MODE = if (args.length > 3) args(3).toInt else 3
    MBTConfigConstants.KNN = if (args.length > 4) args(4).toInt else 8
    MBTConfigConstants.RANGE = if (args.length > 5) args(5).toDouble else 3800
    MBTConfigConstants.LOCAL_M_TREE_INNER_ORDER = if (args.length > 6) args(6).toInt else 3
    MBTConfigConstants.LOCAL_M_TREE_LEAF_ORDER = if (args.length > 7) args(7).toInt else 30
    MBTConfigConstants.BPlusTreeOrder = if (args.length > 8) args(8).toInt else 30
    MBTConfigConstants.GlobalBPlusTreeOrder = if (args.length > 9) args(9).toInt else 3
    ...
    if (MBTConfigConstants.DATA_TYPE == 0) {
      ...
      df1.createMBTIndex(distanceFunction, df1("metric"), "metric_index1")
      ...
      if (INSERT_OR_NOT == 1) {
        for (i <- 0 until SEARCH_TIME) {
          ...
          df1.mbtSimilarityWithRangeSearch(queryPoint, radius, df1("metric"), distanceFunction).show()
        }
        for (i <- 0 until INSERT_NUM) {
          val insertPoint = Point[Any](metricData.takeSample(false, 1).take(1).head.metric)
          df1.mbtInsert(insertPoint, df1("metric"), distanceFunction).show()
        }
        for (i <- 0 until INSERT_NUM) {
          val deletePoint = Point[Any](metricData.takeSample(false, 1).take(1).head.metric)
          df1.mbtDelete(deletePoint, df1("metric"), distanceFunction).show()
        }
      }
      else {
        for (i <- 0 until SEARCH_TIME) {
          val nn = MBTConfigConstants.KNN
          val queryPoint = Point[Any](metricData.filter(t => t.id == (982 + i)).take(1).head.metric)
          df1.mbtSimilarityWithKNNSearch(queryPoint, df1("metric"),
